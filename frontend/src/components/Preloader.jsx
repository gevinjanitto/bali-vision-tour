      ...
      {show && (
        <motion.div key="preloader" className="fixed inset-0 z-[100] bg-forest text-white flex items-center justify-center grain" exit={{ y: '-100%', transition: { duration: 0.8, ease: [0.76, 0, 0.24, 1] } }} data-testid="preloader">
          <div className="relative z-10 flex flex-col items-center">
            <motion.img src={mediaUrl(settings.brand.logoLight || settings.brand.logo)} alt={settings.brand.name} data-testid="preloader-logo" className="w-32 h-16 object-contain" initial={{ scale: 0.6, opacity: 0, rotate: -20 }} animate={{ scale: 1, opacity: 1, rotate: 0 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }} />
            <div className="overflow-hidden mt-5">
              <motion.div className="font-display font-bold text-2xl tracking-tight" initial={{ y: '110%' }} animate={{ y: 0 }} transition={{ duration: 0.7, delay: 0.2, ease: [0.22, 1, 0.36, 1] }} data-testid="preloader-brand">{settings.brand.name}</motion.div>
            </div>

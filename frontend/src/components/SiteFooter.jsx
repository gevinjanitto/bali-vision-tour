...
export const SiteFooter = () => {
  ...
  return <footer className="footer-band grain text-white" data-testid="footer">
    <div className="relative z-10 mx-auto max-w-7xl px-6 lg:px-10 pt-16 pb-8">
      ...
      <div className="mt-14 pt-6 border-t border-white/15 flex flex-col gap-4 text-xs text-white/60">
        ...
        <div className="flex flex-wrap justify-between items-center gap-4"><Link to="/admin/login" className="hover:text-white" data-testid="footer-admin-link">Admin</Link><div className="flex flex-wrap items-center gap-2"><span data-testid="footer-payment-title">{FOOTER_CONTENT.paymentTitle}</span>{FOOTER_CONTENT.payments.map((p, i) => <span key={i} className="rounded-md bg-white/10 px-2.5 py-1 text-[10px] font-bold text-white/85" data-testid={`footer-payment-${i}`}>{p}</span>)}</div></div>
      </div>
    </div>
  </footer>;
};